...
caminho_arquivo = os.path.join(caminho_base, 'data', 'processed', 'salarios_final.csv')
...
with col_graf1:
    if not df_filtrado.empty:
        top_cargos = df_filtrado.groupby('cargo')['salario_dolar'].mean().nlargest(10).sort_values(ascending=True).reset_index()
...
with col_graf3:
    if not df_filtrado.empty:
        remoto_contagem = df_filtrado['regime_trabalho'].value_counts().reset_index()
...
with col_graf4:
    if not df_filtrado.empty:
        ...
        grafico_paises = px.choropleth(media_ds_pais,
            ...
            labels={
                'salario_dolar': 'Salário médio (USD)',
